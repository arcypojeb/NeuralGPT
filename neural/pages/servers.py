...
from ServG4F import WebSocketServer
from ServFire import WebSocketServer1
...
async def main():
    ...
    selectServ = st.selectbox("Select source", ("Fireworks", "GPT4Free", "character.ai"))
    ...
    if selectServ == "GPT4Free":
        ...
        if startServer:
            ...
            serverPorts1.markdown(st.session_state['server_ports'])
            try:      
                server1 = WebSocketServer("localhost", websocketPort)    
                print(f"Starting WebSocket server on port {websocketPort}...")
